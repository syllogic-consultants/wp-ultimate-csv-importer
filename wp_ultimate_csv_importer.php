											...
											<input type="hidden" id="titleduplicatecheck"
												...
												<?php if (isset($_POST['contentduplicatecheck'])) { ?>
													<input type="hidden" id="contentduplicatecheck" name="contentduplicatecheck"
                                                        ...
                                                        <input type="hidden" id="header_array" name="header_array" value="<?php print_r($impCE->headers); ?>"/>
							...
							foreach ($impCE->headers as $key => $value) {
								...
								?>
									...
									foreach ($impCE->defCols as $key1 => $value1) {
										...
										?>
											<option value="<?php print($key1); ?>" <?php if($key1 == $value || $custom_key == $value ): ?>selected="selected"<?php endif; ?>>
											<?php
											if ($key1 != 'post_name'){
												print ($key1);
										...
										?>
											...
											<?php
                                      if($key1 == $value || $custom_key == $value ): $checkCF = 1; endif;
								...
								foreach (get_taxonomies() as $taxokey => $taxovalue) {
									if ($taxokey != 'category' && $taxokey != 'link_category' && $taxokey != 'post_tag' && $taxokey != 'nav_menu' && $taxokey != 'post_format') {
										?>
											<option
											value="<?php print($taxokey); ?>" <?php if($taxovalue == $value ): ?>selected="selected"<?php endif; ?>><?php print($taxovalue);?></option>
											<?php
											$mappingFields_arr[$taxovalue] = $taxovalue;
                                             if($taxovalue == $value ): $checkCF = 1; endif;

									}
								}
								?>
									<option value="add_custom<?php print($count); ?>"  <?php if($checkCF == 0 ): ?>selected="selected"<?php endif; ?> >Add Custom Field
									</option>

									</select> <!-- added to solve issue id 1072-->
									<input class="customfieldtext" type="text"
									id="textbox<?php print($count); ?>"
									name="textbox<?php print($count); ?>" TITLE="Replace the default value"  <?php if($checkCF == 1 ): ?> style="display: none;" <?php endif; ?> value="<?php echo $value ?>"/>
